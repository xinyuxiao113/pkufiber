...
class EqFreqPbcDBP_trainD(nn.Module):
    """
    PBC equalizer.
    ...
    def __init__(self, Nmodes, step, dtaps=2001, ntaps=401, rho=1.0, ol = 40, strides=161, d_share=False, n_share=True, Fs=80e9, D=16.5, Fc: float = 299792458 / 1550e-9, Fi: float = 299792458 / 1550e-9, L=2000e3, gamma=0.0016567):
        super(EqFreqPbcDBP_trainD, self).__init__()
        assert ntaps < ol + strides, "ntaps should be less than ol + strides."
        self.Nmodes = Nmodes
        self.step = step
        self.Fs = Fs
        self.D = D 
        ...
        self.overlaps = ((dtaps - 1)*2 + ol) * step 
        ...
        self.nonlinear = nn.ModuleList([EqStftPBC(M=ntaps, rho=rho, overlaps=ol, strides=strides) for _ in range(n_num)])
...
class EqFreqAMPbcDBP_trainD(nn.Module):
    """
    PBC equalizer.
    M: int, default=41, the length of the filter.
    rho: float, default=1, the parameter of the filter.
    ...
    def __init__(self, Nmodes, step, dtaps=2001, ntaps=401, rho=1.0, ol = 40, strides=161, n_share=True, Fs=80e9, D=16.5, Fc: float = 299792458 / 1550e-9, Fi: float = 299792458 / 1550e-9, L=2000e3, gamma=0.0016567):
        super(EqFreqAMPbcDBP_trainD, self).__init__()
        assert ntaps < ol + strides, "ntaps should be less than ol + strides."
        self.Nmodes = Nmodes
        self.step = step
        self.Fs = Fs
        ...
        n_num = 1 if n_share else self.step
        self.linear = DispersionFilter(step, dtaps, L/step, d_train=True, d_share=False, Fs=Fs, D=D, Fc=Fc, Fi=Fi)
        self.nonlinear = nn.ModuleList([EqStftAMPBC(M=ntaps, rho=rho, overlaps=ol, strides=strides) for _ in range(n_num)])
...
if __name__ == "__main__":
    ...
    model = EqFreqPbcDBP_trainD(Nmodes=2, step=5, dtaps=1001, ntaps=41, rho=0.1)
    y = model(x, task_info)
    print(y.shape)
    # print(model.rmps())
